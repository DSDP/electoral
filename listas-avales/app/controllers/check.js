...
export default Ember.Controller.extend({
	...
	proccesLines: function  (lines ) {
		var _this = this;
		var rNombre = new RegExp(/^([a-z ñáéíóú]{2,60})$/i);
		var rApellido = new RegExp(/^([a-z ñáéíóú]{2,60})$/i);
		var rDni = new RegExp(/^\d{8}$/);
		var rProvincia = new RegExp("");
		var rLocalidad = new RegExp(/^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$/);
		var rSexo = new RegExp(/^M|F$/);
		var rDireccion = new RegExp(/^[a-zA-Z\s\d\/]*\d[a-zA-Z\s\d\/]*$/);
